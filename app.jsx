// Next Level — Ultra Lite + Energy + Per-dot states (static version)
// Uses React globals (no imports). Tailwind for styles.
...
const Btn=({kind="primary",className="",...p})=>(
  <button className={cx(
    ...
    kind==="primary"?"bg-emerald-600/80 border-emerald-500/50 hover:bg-emerald-600":"bg-neutral-800 border-neutral-700 hover:bg-neutral-700",
    className
  )} {...p}/>
);
...
function rangeText(d=new Date()){ const s=weekStartMonday(d), e=weekEndSunday(d); const fmt=(dt)=> dt.toLocaleDateString(undefined,{month:'short',day:'numeric'}); return `${fmt(s)} – ${fmt(e)}`; }
...
function ensurePolarity(m){
  if(!('polarity' in m)){
    const name=(m.name||'').toLowerCase();
    const negHints=['no weed','no alcohol','no drugs','no porn','casual sex','weed','alcohol'];
    m.polarity = negHints.some(h=> name.includes(h)) ? 'neg' : 'pos';
  }
  return m;
}
function computeDayEarned(d){
  let total = 0;
...
function computeWeekTotals(week){
  const maxPerDay=dayPossible(week.templateSnapshot);
  let bestISO='', best=-1, earned=0, possible=0;
  for(const d of week.dayEntries){ const e=computeDayEarned(d); earned+=e; possible+=maxPerDay; if(e>best){best=e; bestISO=d.dateISO;} }
  const pct=possible? (earned/possible)*100:0;
  return {earned:Math.round(earned), possible, pct, bestDayISO:bestISO};
}
const badgeForPct=(p)=> p>=90? 'Platinum' : p>=80? 'Gold' : p>=70? 'Silver' : p>=60? 'Bronze' : undefined;
function applyBonusesAndMulligan(week){
  const maxPerDay=dayPossible(week.templateSnapshot);
  const pctFor=(d)=> maxPerDay? (computeDayEarned({...d,streakApplied:0})/maxPerDay)*100:0;
  let cur=0;
  for(const d of week.dayEntries){
    if(pctFor(d)>=80){
      cur++;
      if(cur>=3){
        const mult=Math.min(1.1+0.05*(cur-3),1.3);
        const base=Object.values(d.pointsByMission).reduce((a,v)=>a+(Array.isArray(v)?v.reduce((aa,x)=>aa+dotScore(x),0):(v||0)),0);
        d.streakApplied=Math.round(base*(mult-1));
      }
    } else cur=0;
  }
  let low=Infinity, idx=0;
  week.dayEntries.forEach((d,i)=>{ const e=computeDayEarned(d); if(e<low){low=e; idx=i;} });
  const sixty=Math.round(maxPerDay*0.6);
  const dd=week.dayEntries[idx];
  if(computeDayEarned(dd)<sixty){ const delta=sixty-computeDayEarned(dd); dd.mulligan=true; dd.earnedBonus=(dd.earnedBonus||0)+delta; week.totals.mulliganUsed=true; }
  const totals=computeWeekTotals(week);
  week.totals={...week.totals, ...totals, badge: badgeForPct(totals.pct)};
}

function toDots(v, weight){
  if (Array.isArray(v)) return v.slice(0,weight).concat(Array(Math.max(0, weight-(v.length||0))).fill(0));
  const filled = Math.max(0, Math.min(weight, Number(v)||0));
  return Array.from({length:weight}, (_,i)=> i<filled?1:0);
}
...
function load(){
  const defaults = () => {
    ...
    return {version: STORE_VERSION, missions, currentWeek: newWeekFromTemplate(missions, new Date()), history: [], settings: { displayName:'Player One', avatarUrl:'', incomeEnabled:false }};
  };
  try{
...
function App(){
  ...
  function cycleDot(missionId, dotIdx){
    setStore(s=>{
      const w=deep(s.currentWeek);
      const i=w.dayEntries.findIndex(d=>d.dateISO===todayISO); if(i<0) return s;
      const m=w.templateSnapshot.find(x=>x.id===missionId); if(!m) return s;
      const arr = toDots(w.dayEntries[i].pointsByMission[missionId], m.weight);
      const cur = arr[dotIdx] ?? 0;
      let next=0;
      if(cur===0) next=1;           // green
      else if(cur===1) next=-1;     // red X (fail)
      else if(cur===-1) next=0.5;   // half
      else next=0;                  // back to empty
      arr[dotIdx]=next;
      w.dayEntries[i].pointsByMission[missionId]=arr;
      return {...s, currentWeek:w};
    });
  }
  function finalize(){
    setStore(s=>{
      const archived=deep(s.currentWeek);
      applyBonusesAndMulligan(archived);
      const nw=newWeekFromTemplate(s.missions,new Date());
      return {...s, history:[archived,...s.history].slice(0,104), currentWeek:nw};
    });
    setTab('report');
  }
  function exportCaption(){
    const w=store.history[0]||store.currentWeek;
    const fmt=(iso)=> new Date(iso).toLocaleDateString(undefined,{month:'short',day:'numeric'});
    const pct=(w.totals?.pct||0).toFixed(1);
    const cap=`Next Level — Week ${fmt(w.startISO)}–${fmt(w.endISO)}\nScore: ${Math.round(w.totals.earned)}/${Math.round(w.totals.possible)} (${pct}%)  Badge: ${w.totals.badge||'—'}\nWins: ${reportNotes.wins||'-'}\nL's: ${reportNotes.losses||'-'}\nLessons: ${reportNotes.lessons||'-'}`;
  ...
  function updateMission(i,patch){
    setStore(s=>{
      const ms=deep(s.missions); ms[i]={...ms[i],...patch};
      const snap=s.currentWeek.templateSnapshot.findIndex(m=>m.id===ms[i].id);
      if(snap>=0){
        s.currentWeek.templateSnapshot[snap]={...ms[i]};
        for(const day of s.currentWeek.dayEntries){
          day.pointsByMission[ms[i].id] = toDots(day.pointsByMission[ms[i].id], ms[i].weight);
        }
      }
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 p-4 md:p-6">
      <header className="flex items-center justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2">
          ...
          <Btn kind="secondary" onClick={()=>setTab('report')}>Report</Btn>
        </div>
      </header>
          ...
          <Card className="md:col-span-2">
            ...
            <div className="p-4">
                  ...
                  <div key={m.id} onClick={()=>setSel(m.id)} className={cx("rounded-xl p-3 border", selected? 'border-emerald-400/40 bg-emerald-400/5':'border-neutral-800 bg-neutral-900/50')}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="font-medium">
                        {m.name} <span className="text-xs opacity-60">(max {m.weight})</span>
                        <span className={cx('ml-2 text-[10px] uppercase px-2 py-0.5 rounded-lg border', m.polarity==='neg'? 'border-red-500/40 text-red-300':'border-emerald-500/40 text-emerald-300')}>{m.polarity==='neg'?'Negative':'Positive'}</span>
                      </div>
                      <div className="text-sm opacity-70">{earned}/{m.weight}</div>
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      {dots.map((v,idx)=>{
                        const base = "relative h-7 w-7 rounded-full border focus:outline-none focus:ring-2 focus:ring-emerald-500 grid place-items-center";
                        let classes = "bg-neutral-800 border-neutral-700";
                        let style = {}; let mark = null;
                        if (v===1){ classes = "bg-emerald-500/90 border-emerald-300/60"; }
                        else if (v===-1){ classes = "bg-red-600/90 border-red-300/60"; mark = <span className="text-[12px] leading-none">×</span>; }
                        else if (v===0.5){ classes = "border-emerald-300/40"; style = { background: "linear-gradient(90deg, rgb(16 185 129 / 0.9) 50%, rgb(38 38 38) 50%)" }; }
                        return <button key={idx} onClick={()=>cycleDot(m.id, idx)} className={cx(base, classes)} style={style}>{mark}</button>;
                      })}
                    </div>
                  </div>
                );})}
              ...
              <div className="mt-4 flex items-center justify-between">
                <div className="text-sm opacity-80">Today: {earnedToday}/{possibleToday}</div>
                <div className="flex gap-2">
                  <Btn kind="secondary" onClick={()=>{ const t=prompt('Quick day note:', currentEntry.note||''); if(t!==null){ setStore(s=>{ const w=deep(s.currentWeek); const i=w.dayEntries.findIndex(d=>d.dateISO===currentEntry.dateISO); w.dayEntries[i]={...w.dayEntries[i], note:t}; return {...s,currentWeek:w}; }); } }}>Add Note</Btn>
                  <Btn onClick={finalize}>Finalize Week Now</Btn>
                </div>
              </div>
            </div>
          </Card>
          ...
          <div className="space-y-4">
              ...
              <div className="p-4 text-sm">
                <div className="flex items-center gap-3">
                  ...
                  <div className="text-xs space-y-1">
                    ...
                    <div className="opacity-70">Negatives reduce energy when missed.</div>
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card>
              ...
              <div className="p-4 space-y-2">
                <Btn kind="secondary" className="w-full" onClick={()=>{ if(confirm('Generate sample?')){ /* no-op here in static cut */ alert('Sample disabled in static cut'); } }}>Generate Sample Data</Btn>
          ...
          <Card className="md:col-span-2">
            ...
            <div className="p-4">
              ...
              <div className="space-y-2">
                ...
                {store.currentWeek.templateSnapshot.filter(m=>m.active && !m.hiddenOnShare).map(m=>{
                  const max=m.weight*7;
                  const earned=store.currentWeek.dayEntries.reduce((a,d)=>{ const arr = toDots(d.pointsByMission[m.id], m.weight); return a + arr.reduce((aa,x)=>aa+dotScore(x),0); },0);
                  return (
              ...
              <div className="mt-3 flex gap-2"><Btn onClick={exportCaption}>Copy Caption</Btn></div>
            </div>
          </Card>
          <Card>
            <div className="p-4 border-b border-neutral-800"><H>About Sharing</H></div>
            <div className="p-4 text-sm opacity-80">Use the caption + a screenshot of this card to post in Skool.</div>
          </Card>
        </div>
      )}
            ...
            {store.missions.map((m,i)=> (
              <div key={m.id} className="rounded-xl border border-neutral-800 p-3 bg-neutral-900/50">
                <div className="grid md:grid-cols-5 items-center gap-2">
                  <Input className="md:col-span-2" value={m.name} onChange={e=>updateMission(i,{name:e.target.value})} placeholder="Mission name"/>
                  <div className="flex items-center gap-2 text-sm">
                    <span>Weight</span>
                    <Select value={m.weight} onChange={e=>updateMission(i,{weight:Number(e.target.value)})}>
                      {[1,2,3,4,5,6,7].map(n=> <option key={n} value={n}>{n}</option>)}
                    </Select>
                  </div>
                  <div className="flex items-center gap-2 text-sm">
                    <span>Type</span>
                    <Select value={m.polarity||'pos'} onChange={e=>updateMission(i,{polarity:e.target.value})}>
